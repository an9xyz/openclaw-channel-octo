  ...
  agentPrompt: {
    messageToolHints: ({ cfg, accountId }: { cfg: any; accountId?: string | null }) => {
      ...
      return [
        ...
        `For sending messages: the target MUST be a concrete id — never a human-readable name. Use target="group:<groupId>" for a group (中文: 群), target="group:<group_no>____<short_id>" (four underscores) for a sub-topic / thread (中文: 子区 / 话题 / thread), or target="user:<userId>" for a 1v1 direct message. If sending to the current conversation, no prefix is needed.`,
        `When the user identifies a destination by NAME (e.g. "转发到 'XXX' 群") instead of an explicit id, the name is AMBIGUOUS — "XXX" could refer to either a group (群) or a sub-topic (子区). DO NOT guess and DO NOT send the name as the literal target. Resolve it FIRST: call octo_management action="channel-list" and match the name against group names; if no group matches, call action="list-threads" with target="group:<candidate>" on the relevant candidate group(s) to find a sub-topic match. Send only after the name resolves to a SINGLE concrete id. If two or more candidates match (e.g. a group and a sub-topic share the name, or sub-topics across different groups share the name), reply to the user with the candidate list and ask which one BEFORE sending — never silently pick one. Issue #101.`,
