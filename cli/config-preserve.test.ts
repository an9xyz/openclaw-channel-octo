...
describe("restoreChannelConfigToFile", () => {
  ...
  it("should write channels.octo back to file with backup", async () => {
    ...
    expect(writtenContent.channels.octo.accounts.my_bot.botToken).toBe("bf_real_secret_token");
...
describe("config preserve on install --force failure", () => {
  it("should restore config even when pluginsInstall throws", async () => {
    ...
    expect(restored.channels.octo.accounts.my_bot.botToken).toBe("bf_real_secret_token");
  });
});
