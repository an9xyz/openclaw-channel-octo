/**
 * doctor command: diagnose and optionally fix Octo plugin health.
 *
...
export async function runDoctorChecks(params: {
  ...
  if (!params.inProcess && fix) {
    ...
    // Phase 0b: Clean up stale install stage directories (Octo only, >10min old)
    ...
    if (cfg) {
      ...
      if (octoBindings.length > 0 && !hasOctoChannel) {
        ...
        if (orphaned.length > 0) {
          removeOrphanedBindingsFromFile(CHANNEL_ID, configuredAccounts);
          checks.push({
            name: "Orphaned bindings",
            status: "FIXED",
  ...
  if (!params.inProcess) {
    ...
    if (enabled === "true") {
      ...
      try {
        configSet(`plugins.entries.${PLUGIN_ID}.enabled`, "true");
  ...
  const accounts = reader.getJson(channelConfigPath("accounts"));
  const accountIds = accounts ? Object.keys(accounts) : [];
  const legacyToken = reader.get(channelConfigPath("botToken"));
  ...
  for (const acctId of targetAccounts) {
    ...
    const tokenPath = isLegacy
      ? channelConfigPath("botToken")
      : channelConfigPath("accounts", acctId, "botToken");
    ...
    const apiUrlPath = isLegacy
      ? channelConfigPath("apiUrl")
      : channelConfigPath("accounts", acctId, "apiUrl");
    let apiUrl = reader.get(apiUrlPath);
    if (!apiUrl) apiUrl = reader.get(channelConfigPath("apiUrl"));
...
export function formatDoctorResult(result: DoctorResult): string {
  const lines = ["Octo Plugin Doctor"];
