...
export async function runQuickstart(opts: QuickstartOptions): Promise<void> {
  ...
  try {
    ...
    // OpenClaw stdout may contain plugin log noise before JSON (e.g. [octo], [plugins]).
  ...
  for (const bot of successful) {
    // Account config
    channelConfig.accounts[bot.robotId] = {
    ...
    const existingIdx = (cfg.bindings as any[]).findIndex(
      (b: any) => b.match?.channel === CHANNEL_ID && b.match?.accountId === bot.robotId,
    );
    if (existingIdx >= 0) {
      ...
      cfg.bindings.push({
        agentId: bot.agentId,
        match: { channel: CHANNEL_ID, accountId: bot.robotId },
      });
    }
  }
  ...
  // 7. Output results
  console.log("\n========================================");
  console.log("Quickstart complete! Send a message to each bot in Octo to verify.");
  console.log("========================================\n");
